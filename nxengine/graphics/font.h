...
public:
	...
	bool InitKrChars(SDL_Surface *font, uint32_t color);
	bool InitKrCharsShadowed(SDL_Surface *top, uint32_t color, uint32_t shadowcolor);
	void free();
	
	SDL_Surface *letters[NUM_FONT_LETTERS];
	SDL_Surface *krletters[NUM_FONT_KR_LETTERS];
};
